...
public class payStartMenu extends JPanel {
	...
	private JPanel paystartMenu;

	public payStartMenu(MainFrame mf) {

		/*현재 프레임 및 클래스 set*/
		this.mf = mf;
		this.paystartMenu = this;
		...
		/* 라벨에 타이틀로고 삽입 */
		JLabel logo = new JLabel(new ImageIcon("images/select/홀짝게임라벨.png"));
		...
		quitBtn.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				changePanel(mf, paystartMenu, new GameMenu(mf));
		...
		payrankBtn.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				changePanel(mf, paystartMenu, new payrank(mf));
				System.out.println("GameMenu로 이동합니다.");
